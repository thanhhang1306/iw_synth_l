...
# top-n best and worst per metric
fig, axes = plt.subplots(nrows=len(METRICS), ncols=2, figsize=(14, 3*len(METRICS)))
plt.subplots_adjust(hspace=0.6, wspace=0.4)
for i, metric in enumerate(METRICS):
    df['label'] = df['variant'].astype(str).apply(lambda x: f"({x})") + '\n' + df['experiment']
    # best
    best = df.nlargest(TOP_N, metric)
    bars = axes[i,0].bar(best['label'], best[metric], color='tab:blue')
    axes[i,0].bar_label(bars, fmt='%.3f', padding=3)
    ...
    bars2 = axes[i,1].bar(worst['label'], worst[metric], color='tab:red')
    axes[i,1].bar_label(bars2, fmt='%.3f', padding=3)
...
fig.tight_layout()
out = os.path.join(OUT_DIR, 'figure_top10_best_worst.png')
...
# mean performance by variant (2×3 grid)
mean_df = (
...
for j, metric in enumerate(METRICS):
    ...
    sns.barplot(
        ...
        color=palette[j], width=0.6, ax=ax
    )
    ax.margins(x=0.1)
    for p in ax.patches:
        h = p.get_height()
...
# weighted_f1 distribution by augmentation
...
# per-class heatmaps, confusion gallery, precision-recall
...
for ax, (_, row) in zip(axes, best.iterrows()):
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='OrRd', xticklabels=class_labels, yticklabels=class_labels, ax=ax)
...
for ax, cls in zip(axes, [1,2,3,4]):
    ...
    sns.scatterplot(data=pts, x='recall', y='precision', hue='variant', style='experiment', ax=ax, palette='Set3', s=100)
